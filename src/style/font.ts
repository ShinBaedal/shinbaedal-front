const medium = 500;
const regular = 400;
export const font = {
  headline1: `bold 32px 'Spoqa Han Sans Neo', 'sans-serif'`,
  headline2: `bold 28px 'Spoqa Han Sans Neo', 'sans-serif'`,
  headline3: `$bold 24px 'Spoqa Han Sans Neo', 'sans-serif'`,
  subtitle1: `bold 20px 'Spoqa Han Sans Neo', 'sans-serif'`,
  subtitle2: `${medium} 20px 'Spoqa Han Sans Neo', 'sans-serif'`,
  body1: `bold 16px 'Spoqa Han Sans Neo', 'sans-serif'`,
  body2: `${medium} 16px 'Spoqa Han Sans Neo', 'sans-serif'`,
  body3: `${regular} 16px 'Spoqa Han Sans Neo', 'sans-serif'`,
  description1: `bold 14px 'Spoqa Han Sans Neo', 'sans-serif'`,
  description2: `${regular} 14px 'Spoqa Han Sans Neo', 'sans-serif'`,
  description3: `${regular} 12px 'Spoqa Han Sans Neo', 'sans-serif'`,
};
